...
```
## mybtaisplus联表查询
在我们查询的时候即需要wrapper中的条件对象又需要自定义sql我们如何将wrappert条件中到传入到sql中呢？
我们可以使用以下方式
```java
@Select("select * from A ${ew.customeSqlSegment}")
Ipage<IpageDTO> selectPage(@Param("page") Ipage page,@Param(Constants.WRAPPER) Wrapper<> wrapper);
